...
def lee_escrutinio(fichero_votos, fichero_escaños):
    ...
    with open(fichero_votos, encoding='utf-8') as f:
        lector_votos = csv.reader(f)  
        next(lector_votos)
        votos = [(provincia, partido, int(voto)) for provincia, partido, voto in lector_votos]
    
    
    with open(fichero_escaños, encoding = 'utf-8') as g:
        lector_escaños = csv.reader(g)
        next(lector_escaños)
        diccionario_escaños = dict()
        for e in lector_escaños:
            diccionario_escaños[e[0]] = e[1]
    
    return (votos, diccionario_escaños)    
...
def calcula_diccionario_provincia(votos, provincia):
    ...
    diccionario_votos_por_provincias = dict()
    
    for e in votos:
        if e[0] == provincia:
            diccionario_votos_por_provincias[e[1]] = e[2]
    
    return diccionario_votos_por_provincias
...
def calcula_diccionario_provincias(votos):
    ...
    diccionario_provincia = dict()
    
    for e in votos:
        provincia = e[0]    
        diccionario_provincia[provincia] = calcula_diccionario_provincia(votos, provincia)
    
    return diccionario_provincia
...
def totales_por_partido(tabla_valores):
    ...
    for e in tabla_valores:
        partidos_votos = tabla_valores[e]
        for a in partidos_votos:
            partido = a
            votos = partidos_votos[a]
            if partido in suma_votos:
                suma_votos[a] += votos
            else:
                suma_votos[a] = votos
    return suma_votos
    ...
    for prov in tabla_valores:
        dict_partido_votos = tabla_valores[prov]
        A = Counter(dict_partido_votos)
        B = A+B
    return dict(B)
...
def genera_diagrama_tarta(diccionario, limite=None):
    ...
    diccionario2 = sorted(diccionario, reverse=True, key=diccionario.get)
    
    for e in diccionario2[:limite]:
        valores.append(diccionario[e])
        claves.append(e)
    
    plt.pie(valores, labels=claves, autopct='%1.1f%%', startangle=90)
    plt.axis('equal')
    plt.show()
