...
def filtrar_por_año(preguntas, año):
    ...
    #preguntas = [(puntuacion, titulo, anyo, etiqueta) for puntuacion, titulo, anyo, etiqueta in preguntas if anyo==año]
    preguntas=[(e.puntuacion, e.titulo, e.año, e.etiqueta) for e in preguntas if e.año==año]
    return preguntas
...
def calcular_etiquetas(preguntas):
    ...
    '''
    etiquetas = {etiquetas for _,_,_,etiquetas in preguntas}
    return etiquetas
...
def calcular_preguntas_mejor_valoradas(preguntas, limite=10):
    ...
    SALIDA: 
       - lista de tuplas (titulo, puntuacion) ordenanda de mayor a menor puntuacion  -> [(str, int)]
    '''
    #puntuacion=[(titulo, puntuacion) for puntuacion, titulo,_,_ in preguntas]
    puntuacion=[(e.titulo, e.puntuacion) for e in preguntas ]
    puntuacion=sorted(puntuacion, reverse=True, key=lambda t:t[1])
    return puntuacion[:limite]
...
def contar_etiquetas(preguntas):
    ...
    for datos in preguntas:
        etiqueta=datos[3]
        contador=0
        for datos2 in preguntas:
            if etiqueta==datos2[3]:
                contador=contador+1
...
def mostrar_distribucion_etiquetas(preguntas, etiquetas):
    ...
    etiquetas2 = [etiqueta for _,_,_, etiqueta in preguntas if etiqueta in etiquetas]

    # Tenemos guardadas todas las etiquetas sin repetir
    ...
    for e in etiquetas:
        contador=0
        etiqueta=e
        for etiq in etiquetas2:
            if etiq == etiqueta:
                contador+=1
        tamaños.append(contador)
...
def contar_palabras_clave(preguntas, stopwords=[]):
    ...
    ENTRADA: 
       ...
       - stopwords: palabras huecas, consideradas no relevantes como palabras clave
    SALIDA: 
       - lista de tuplas (termino, frecuencia) ordenanda de mayor a menor frecuencia  -> [(str, int)]
    '''
    pass
