...
def lee_poblaciones(fichero):
    ...
    with open(fichero, encoding='utf-8') as f:
        ...
        poblaciones = [(str(nombre), str(codigo), int(anyo), int(censo)) for nombre, codigo, anyo, censo in lector]     
        return poblaciones
...
def calcula_paises(poblaciones):
    ...
    paises=[str(nombre) for nombre,_,_,_ in poblaciones]
    conjunto=set(paises)
    paises=list(conjunto)
    return sorted(paises)
##############################################################################################
...
def filtra_por_pais(poblaciones, pais):
    ...
    filtrado=[(int(anyo), int(censo)) for nombre,_,anyo,censo in poblaciones if nombre==pais]
    return filtrado
##############################################################################################

############################################################################################## 
def filtra_por_paises_y_anyo(poblaciones, anyos, paises):
    ...
    panyo=[(nombre, censo) for nombre,_,anyo,censo in poblaciones if (nombre in paises and anyo==anyos)]
    return panyo
...
def muestra_evolucion_poblacion(poblaciones, pais):
    ...
    #muestra=[(l_anyos, l_habitantes) for nombre,_,anyo,censo in poblaciones if nombre==pais]
    
    l_anyos=[]
    l_habitantes=[]
    
    for muestra in poblaciones:
        if muestra[0]==pais:
            l_anyos.append(muestra[2])
            l_habitantes.append(muestra[3])
    
    
    plt.title("Evolución de la población según el año")
    plt.plot(l_anyos,l_habitantes)
    plt.show()
...
def muestra_comparativa_paises_anyo(poblaciones, anyo, paises):
    ...
    for muestra in poblaciones:
        if muestra[2]==anyo and muestra[0] in paises:
                l_paises.append(muestra[0])
                l_habitantes.append(muestra[3])
