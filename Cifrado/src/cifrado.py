...
def codificacion(k):
    codificacion = construye_diccionario_codificacion(k)
    return codificacion
...
def construye_diccionario_codificacion(k):
    ...
    for i, a in enumerate(alfabeto):
        letra_codificada = alfabeto[((i+k) % len(alfabeto))]
        diccionario[a]=letra_codificada
    return diccionario
...
def cifra_mensaje(texto, k):
    ...
    codificado = codificacion(k)
    mensaje_cifrado = ''
    
    for a in texto:
        if a in codificado:
            mensaje_cifrado += (codificado[a])
        else:
            mensaje_cifrado += a
...
def descifra_mensaje(texto, k):
    ...
    descodificado = codificacion(-k)
    mensaje_descifrado = ''
    
    for a in texto:
        if a in descodificado:
            mensaje_descifrado += descodificado[a]
        else:
            mensaje_descifrado += a
    
    return mensaje_descifrado
    
#Para ahorrarte el if hariamos
    # letra_cifrada = codificacion.get(letra , letra)
    # busca si exsiste la letra en el diccionario codificacion  "codificacion.get(letra ,  )"
    # Si no existe por defecto daria la misma letra "codificacion.get(   , letra)" 
...
def es_palabra_valida(palabra, vocabulario):
    ...
    for a in vocabulario:
        b = a.lower()
        b = b.strip(" !@#$%^&*()-_+={}[]|\:;'<>?,./\"¡¿") # Elimina dichos elementos del str
        vocabulario2.add(b)
        
    if palabra in vocabulario2:
        return True
    else:
        return False
...
def cuenta_palabras_validas(texto, vocabulario):
    ...
    lista_palabras = set()
    texto = texto.lower() # Para convertir todo en minúscilas
    texto = texto.split() # Para delimitar los conjuntos según los espacios (por defecto)
    
    
    for a in texto:
        if a in vocabulario:
            lista_palabras.add(a)
...
def rompe_codigo_cifrado(texto, vocabulario):
    ...
    while count < count_max:
        mensaje_descifrado = descifra_mensaje(texto, k)
        palabras_validas = cuenta_palabras_validas(mensaje_descifrado, vocabulario)
        tupla = (palabras_validas, mensaje_descifrado)
        lista.append(tupla)
        count += 1
        k += 1
