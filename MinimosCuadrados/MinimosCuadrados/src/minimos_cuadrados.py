...
def lee_registros(fichero):
    ...
    with open(fichero, encoding='utf-8') as f:
        lector = csv.reader(f)
        registro=[(genero, float(edad), float(altura), float(peso)) for genero, edad, altura, peso in lector]
        return registro
...
def pesos_alturas_por_edades(registros, edad_min, edad_max):
    ...
    peso_altura_edad = [(peso, altura) for _,edad ,altura, peso in registros if (edad_min <= edad and edad <= edad_max)]
    return peso_altura_edad
...
def pesos_alturas_por_genero(registros, genero):
    ...
    peso_altura_genero = [(peso, altura) for generos,_, altura, peso in registros if genero==generos]
    return peso_altura_genero
...
def calcula_recta_regresion(puntos):
    ...
    lista_puntos_x = [x for x, _ in puntos]
    lista_puntos_y = [y for _, y in puntos]
    lista_puntos_xy = []
    lista_puntos_xx = []
    
    n=len(lista_puntos_x) #Dado que x e y tienen el mismo numero de elementos
    
    for x,y in puntos:
        multi = x*y
        lista_puntos_xy.append(multi)
        cuadrado = x*x
        lista_puntos_xx.append(cuadrado)
    
    sum_x = sum(lista_puntos_x)
    sum_y = sum(lista_puntos_y)
    sum_xy = sum(lista_puntos_xy)
    sum_xx = sum(lista_puntos_xx)
...
def evalua_metrica_MAE(puntos, a, b):
    ...
    prediccion = []
    puntos_y = [y for _, y in puntos]
    error = []
   
    for x, y in puntos:
        py = a*x + b
        prediccion.append(py)
    
    y_prediccion = zip(puntos_y, prediccion)
          
    for y, py in y_prediccion:
        calculo = abs(y-py)
        error.append(calculo)
